...
For instance, suppose a gym has an initial signup fee of $\\$50$ and monthly membership fee of $\\$20$.  In this scenario, the initial signup fee is the y-intercept and the monthly fee is the slope.  Using $m=20$ and $b=50$ the equation of the linear function is:
\[ y = 20x + 50 \]
...
10 &= k (6)^{0.6309} \cdot \ln (3) && \small \color{#5fa2ce}{\text{Substitute $p=0.6309$}} \\newline
...
With both $k$ and $p$ in hand, we can now write the equation our power function. 

\[
  f(x)=3.229x^{0.6309}
\]

Keep in mind that, since power functions generally exist only when $x \gt 0$, we should check that both $x$-cooridnates are positive before starting this process.  Additionally, since logarithms were involved in our solving process, the two $y$-coordinates should be non-zero and have the same sign.  For the most part, expect to find power functions only when both points are in the first quadrant.
...
The answer is that we certainly could have.  The format we use doesn't really matter since its possible to switch between any of them.  In particular, the standard model $f(x)=a \thinspace b^{x}$ and the continuous growth model $f(t)=a \thinspace e^{k t}$ are equivalent (and are the most commly used).  Here's how you convert from one to the other.
