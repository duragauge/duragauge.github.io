...
{{% check %}}
1. If we had solved Equation \#2 for $a$ at the start, we would have gotten $a = \frac{27}{b^3}$.  Substitute that expression into Equation \#1.
{{% answer %}}
\[
\begin{align}
12 &= a  \thinspace  b^{1} && \small \color{#5fa2ce}{\text{Equation #1}} \newline
...
1. Suppose you were looking for a power function through the points $(2, 5)$ and $(6,10)$.  Write the system of equations that would result from inserting the values from those points into the power function format $f(x)=k \thinspace x^{p}$ .
{{% answer %}}
\[
\begin{align}
