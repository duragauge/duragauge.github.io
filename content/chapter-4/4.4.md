...
{{% check %}}
1. Use this shoes-and-socks method to find the inverse of $g(x)=2x-7$.
{{% answer %}}$g^{-1}(x)=\frac{x+7}{2}${{% /answer %}}
{{% /check %}}
...
To find the inverse using the switch-and-solve method we first rewrite the function as the equation $y=\frac{x^{3}+1}{5}$, with $y$ in place of $f(x)$.  
...
| **Step 1:** | Replace $f(x)$ with $y$ | {{% answer %}}$y=-5x+8${{% /answer %}} |
| --- | --- | --- |
| **Step 2:** | Switch $x$ and $y$ | {{% answer %}}$x=-5y+8${{% /answer %}} |
| **Step 3:** | Solve for $y$ | {{% answer %}}Subtract $8$ to get $x-8=-5y$.<\br> Divide by $-5$ to get $y=\frac{x-8}{-5}$. <\br> Simplify to $y=\frac{8-x}{5}${{% /answer %}} |
| **Step 4:** | Replace $y$ with $f^{-1}(x)$ | {{% answer %}}$f^{-1}(x)=\frac{8-x}{5}${{% /answer %}} |
...
Since the range of $f$ is $y\geq6$, we must restrict the domain of $f^{-1}$ to $x\geq6$.  This means we should choose $-(x-6)^{3/2}$ as the equation for $f^{-1}$.
...
{{% check %}}
1.  Suppose the *range* of a one-to-one function $f$ is known to be $[3, 18]$.  What is the *domain* of $f^{-1}$?
{{% answer %}}The domain of $f^{-1}$ is $[3,18]$.{{% /answer %}}
{{% /check %}}
...
{{% check %}}
1.  Are the functions $\color{red}{f(x)=\frac{x}{2}+3}$ and $\color{blue}{g(x)=2x-3}$ inverses?  Use the round trip properties to justify your answer.
{{% answer %}}No, these are not inverses.  $$(f \circ g)(x)=\color{red}{ \frac{ \color{blue}{2x-3}}{2} + 3 } = x + \frac{3}{2}$$ and $$(g \circ f)(x) = \color{blue}{2\left(\color{red}{\frac{x}{2}+3}\right)-3}=x+3$$  Neither of these equal $x$, so the functions cannot be inverses.{{% /answer %}}
{{% /check %}}
...
{{% check %}}
1. Evaluate $f^{-1}(1000)$ and describe what it would mean to the scientists in Antartica.
{{% answer %}}
