...
{{% check %}}
1.  Suppose the example above had started with $x = 1$ instead.  What would be the result of the composition?
{{% answer %}}Putting $x =1$ into $\color{blue}{g}$ gives $\color{blue}{g(}1\color{blue}{)}=\color{blue}{3}$.  Putting $\color{blue}{3}$ into $\color{red}{f}$  produces  $\color{red}{f(}\color{blue}{3}\color{red}{)}=\color{red}{30}$.  So $\color{red}{f(}\color{blue}{g(}1\color{blue}{)}\color{red}{)}=30$ is the result of the composition.{{% /answer %}}
{{% /check %}}


## 4.3.5 Understand Composition of Functions
...
{{% check %}}
1.  Using the functions above, find the Social Security deduction on the profits from selling $1000$ cupcakes.
{{% answer %}}First find $$P(1000)=1.85(1000) - \frac{0.85}{1000}\approx\$1850$$
...
{{% check %}}
1.  Using the same functions as above, find $(f \circ g)(7)$.   {{% answer %}}
\[
\begin{align}
(f \circ g)(7) &= f(g(7) \newline
...
Let's choose two simple functions to work with $\color{red}{f(x)=x+4}$ and $\color{blue}{x^2}$.
...
$h(x)=(f \circ g)(x)$ we conclude that the domain is the set $\\{2,4,5\\}$.

Notice again that the domain of $h=f \circ g$ is always a subset of the domain of $\color{blue}{g}$.


{{% check %}}
1.  Using the same function tables as above, what is the domain of $(g \circ f)(x)$?   {{% answer %}}The domain of $(g \circ f)(x)$ is the set {-2,0,1}.{{% /answer %}}
{{% /check %}}
...
{{% check %}}
1.  Using the same functions as above, verify that $(f \circ g)(1)$ gives the same value as $h(1)$.   {{% answer %}}
\[ \begin{align}
(f \circ g)(1)&=f(g(1)) \newline
&=f(2^1) \newline
&=f(2) \newline
...
<iframe scrolling="no" src="https://www.geogebra.org/material/iframe/id/Kbg4KOk8/width/850/height/400/border/888888/rc/false/ai/false/sdz/false/smb/false/stb/false/stbh/false/ld/false/sri/true/at/preferhtml5" width="850px" height="400px" style="border:0px;"></iframe>
