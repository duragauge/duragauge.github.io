...
Evaluating a function is different than solving an equation.  When **solving** we are given an output and must find the corresponding input.  To solve $f(x)=5$, for instance, we scan the table in the opposite direction, looking for an $x$ that creates $5$ as an output.  The solution is $x=0$.
