...
In a very similar way, we can take basic functions and transform them into new functions by adding constants into the mix.  As you gain experience you will be able to predict the behavior of a function without graphing it.  You will also learn to construct the equation for a function given its graph.
...
{{% geogebra ratio="57.69%" id="Y5TzgBaP" %}}
</br>
The result of adding a constant to a function is that the graph moves up or down $\color{blue}{c}$ spaces.

More formally, if $\color{red}{f}$ is any function and $\color{blue}{c}$ is any constant, then the graph $\color{red}{f(x)}  \color{blue}{+ c}$ is a **vertical shift** of the function $f$.  The graph of $\color{red}{f(x)}  \color{blue}{+ c}$ can be found by adding $\color{blue}{c}$ to every y-value of the original graph.  In other words,
...
{{% geogebra ratio="51.92%" id="YJUZlyHJ" %}}
</br>
You have just seen that the graph of $\color{red}{f(x } \color{blue}{+ c}\color{red}{)}$ is a **horizontal shift** of the function $f$.  The graph of $\color{red}{f(x } \color{blue}{+ c}\color{red}{)}$  can be found by subtracting $\color{blue}{c}$ from every x-value of the original graph.  In other words,
...
{{% geogebra ratio="74.07%" id="HJhxc4Lx" %}}
...
How much wider is the function when $\color{blue}{c = 2}$  When $\color{blue}{c = 0.5}$?

{{% geogebra ratio="50%" id="ZPBnPTlS" %}}
<br>
...
![](/img/chapter-1/golf_drive.gif#center) Imagine that a new club has been developed that will transform $f(x)$ into $f(2 x)$.  What would this new club do to the distance of his drive?
{{% answer %}}$f(2 x)$ is a horizontal stretch with $c=2$.  Since $c=2>1$, this will actually compress the graph horizontally, cutting the distance in half. <br> ![](/img/chapter-1/golf_drive_answer.gif)<br>  If the goal was to double the distance, the transformation would need to be $f(\frac{1}{2}x)$.{{% /answer %}}
1. What function would look like $f(x)=x^{2}$ but horizontally stretched by a factor of $3$?{{% answer %}}$f(\frac{x}{3}) = (\frac{x}{3}^2)${{% /answer %}}
{{% /check %}}
...
<iframe scrolling="no" src="https://www.geogebra.org/material/iframe/id/ae4EaVFq/width/450/height/300/border/888888/rc/false/ai/false/sdz/false/smb/false/stb/false/stbh/false/ld/false/sri/false/at/auto" width="450px" height="300px" style="border:1px;"> </iframe></div><br>
...
<div style="text-align:center">
<iframe scrolling="no" src="https://www.geogebra.org/material/iframe/id/xQwb021E/width/450/height/300/border/888888/rc/false/ai/false/sdz/false/smb/false/stb/false/stbh/false/ld/false/sri/false/at/auto" width="450px" height="300px" style="border:1px;"> </iframe></div><br>
...
| $f(\color{blue}{c} \thinspace x)$ | Horizontal Stretch | $(x, y) \rightarrow (\frac{x}{\color{blue}{c}}  \thinspace  , \thinspace y)$ |
...
{{% check %}}
1. What is the difference between $2f(x)$, $ f(2x)$ and $ f(x)+2$ ? {{% answer %}}$2f(x)$ means to multiply each $f(x)$ value by $2$, resulting in a vertical stretch by a factor of $2$.</br></br> $f(2x)$ means to use $2 x$ as the input to the function $f$.  The result is a  horizontal compression by a factor of $1/2$. </br></br>$f(x)+2$ means we add $2$ to each $f(x)$ value, which shifts the graph vertically $2$ spaces.</br>{{% /answer %}}
{{% /check %}}



## Symmetry
Combining a horizontal reflection with a vertical reflection creates a **reflection around the origin**, which plays a large role in function symmetry.
...
<iframe scrolling="no" title="Explore Symmetry" src="https://www.geogebra.org/material/iframe/id/iHmotfOF/width/320/height/400/border/fafafa/rc/false/ai/false/sdz/false/smb/false/stb/false/stbh/false/ld/false/sri/false/at/auto" width="320px" height="400px" style="border:0px;"> </iframe>


## Even and Odd Symmetry
When reflecting a graph, two interesting things happen sometimes.  The first is that a horizontal reflection across the y-axis might  produces the same exact graph as the original function.  Symbolically, $f(-x)=f(x)$.  When this happens we say that the function has **even symmetry**, or is symmetric across the y-axis.   The figure below illustrates the concept.

<iframe scrolling="no" title="Even Symmetry" src="https://www.geogebra.org/material/iframe/id/mSp9DhKO/width/300/height/300/border/fafafa/rc/false/ai/false/sdz/false/smb/false/stb/false/stbh/false/ld/false/sri/false/at/auto" width="300px" height="300px" style="border:0px;"> </iframe>
...
<iframe scrolling="no" title="Odd Symmetry" src="https://www.geogebra.org/material/iframe/id/DpMbG1ID/width/370/height/370/border/fafafa/rc/false/ai/false/sdz/false/smb/false/stb/false/stbh/false/ld/false/sri/false/at/auto" width="370px" height="370px" style="border:0px;"> </iframe>
...
## Looking Ahead
Transformations will be helpful when we model data in [Section 1.8](../1.8).  If a data set looks like a line, for example, then we can apply shifts and stretches to the identity function $f(x)=x$ until we find a combination that fits the shape of the data.
