...
All that is needed to create a set is a way of deciding if an object is part of the collection or not.
...
is the set of colors on a traffic light.{{% sidenote "note about braces" %}}Sets are always enclosed in curly braces $\lbrace \thinspace \thinspace \rbrace$.{{% /sidenote %}}
...
\[
  \left\lbrace \text{my favorite things} \right\rbrace
\]
...
\[
  \left\lbrace x \thinspace \vert \thinspace x \text{ is something in your backpack} \right\rbrace
\]
...
The union of some set $A$ with another set $B$ is written using the union symbol: $\bigcup$.
