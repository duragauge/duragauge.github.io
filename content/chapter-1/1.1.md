...
According to legend, Hippasus was trying to measure the diagonal of a 1 by 1 square{{% sidenote "sqrt_2" %}}![a 1-by-1 square with the diagonal labled as sqrt{2}](/img/chapter-1/sqrt_2.svg#center){{% /sidenote %}}.  He knew from the Pythagorean theorem that the answer was $\sqrt{2}$, but had been unable to find a rational number that was equal to $2$ when you square it.
