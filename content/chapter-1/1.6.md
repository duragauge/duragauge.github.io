...
The formulas and tables we have used so far can only show what happens on a limited number of intervals.  What would be very helpful is a mechanism that has the potential to describe the behavior of the function on any interval all at once.

![](/img/chapter-1/difference_quotient.svg#center)
The tool that does this is called the **difference quotient**.  The difference quotient $D(x)$ is a function that lets us calculate the average rate of change on a generic interval from $x$ to  $\color{blue}{x+h}$, where  $\color{blue}{h}$  is any positive number.  

To find the formula for the difference quotient we simply take the average rate of change formula and substitute $x$ in place of  $a$, and $\color{blue}{x+h}$  in place of  $\color{blue}{b}$.  
