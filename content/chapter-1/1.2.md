...
Solving is the reverse process.  As an example, to solve $f(x)=4$ for $x$ you move the  **INPUT** until the **OUTPUT** equals $4$.  Then you record the **INPUT**.  Here there are multiple values for $x$ that work: $x=1.7$, $x=4$, and $x=11.7$ all appear to give outpus of $4$, and would be valid solutions to $f(x)=4$.
