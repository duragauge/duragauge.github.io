...
The first new tool is the **rational zeros theorem**.  The rational zeros theorem comes from a nice pattern that occurs whenever the zeros of a polynomial happen to be fractions.  Inspect the table below and see if you can spot a pattern.
...
We can use that pattern to create a list of fractions that *might* be zeros.  Those fractions could then be tested to see if any of them really are zeros.
...
1. List all the fractions whose numerators come from factors of $\color{red}{6}$ with denominators that are factors of $\color{blue}{5}$. {{% answer %}}There are sixteen combinations.
